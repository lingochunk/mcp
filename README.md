...
|---|---|---|
| `get_vocabulary` | `vocab:read` | Your vocabulary, aggregated per word with FSRS maturity (known/learning/new/due). Filterable; incremental sync via `since` + `cursor`. |
...
| `search_examples` | `content:read` | Example sentences across your library, by word (`lemma`) or text (`q`). |
| `get_audio_clip` | `content:read` | Cuts a short native-audio snippet and **saves it to a local file**, returning the path, for embedding in lessons. |

Plus the **`lingochunk-lesson`** skill, which builds a single self-contained HTML
lesson (data-URI audio; gap-fill, multiple-choice, listening and blur-reveal
exercises) from the tools above, filtering out words you already know.

## Prerequisites

- Node.js >= 18.
- A LingoChunk **personal access token**: in LingoChunk, go to
  Settings -> API tokens, create one, and grant the scopes you need
  (`vocab:read` and `content:read` cover everything here). The token is shown
  once and starts with `lcp_`.

## Use it

### Option A - Claude Code plugin (bundles the server + the lesson skill)

Build once, then point Claude Code at this directory as a plugin. The bundled
`.mcp.json` runs the server from `dist/` via `${CLAUDE_PLUGIN_ROOT}`.
...
Until then, run the local build directly:
...
| `LINGOCHUNK_API_URL` | no | `https://lingochunk.com` | API origin override (for self-host/testing). |
| `LINGOCHUNK_OUTPUT_DIR` | no | OS temp dir `/lingochunk-mcp` | Where `get_audio_clip` writes clip files. |
...
skills/lingochunk-lesson/  the lesson skill + self-contained HTML template
recipes/fluent.md       how to plug this into the `fluent` tutor plugin
openapi/openapi.json    the committed public API spec (the contract this wraps)
...
```bash
npm install        # deps + build
npm run build      # compile src/ -> dist/
npm run typecheck  # type-check without emitting
```

The committed `openapi/openapi.json` is the source contract; it is exported from
the LingoChunk repo (`make generate-openapi-public`) and refreshed here on each
API release.
